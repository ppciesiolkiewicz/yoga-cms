...
export default async function SiteDetailPage({ params }: Params) {
  ...
  try {
    result = await repo.getJson<ResultFile>({
      requestId,
      stage: "",
      name: "result.json",
    });
  ...
  const techMap = (siteData.artifacts["detect-tech"] ?? {}) as Record<
    string,
    TechArtifact
  >;
  const lighthouseMap = (siteData.artifacts["run-lighthouse"] ?? {}) as Record<
    string,
    LighthouseArtifact
  >;
  const extractMap = (siteData.artifacts["extract-pages-content"] ??
    {}) as Record<string, ExtractArtifact>;
  const progressMap = (siteData.artifacts["progress"] ?? {}) as Record<
    string,
    CategoryProgress
  >;
  ...
  const classify =
    (
      siteData.artifacts["classify-nav"] as
        | { byCategory: Record<string, string[]> }
        | undefined
    )?.byCategory ?? {};

  const report = siteData.artifacts["build-report"] as
    | { scrapedAt?: string }
    | undefined;
  ...
  const sidebarSites = result.request.sites.map((s) => {
    ...
    const siteTechMap = (siteData?.artifacts["detect-tech"] ?? {}) as Record<
      string,
      TechArtifact
    >;
    const siteLhMap = (siteData?.artifacts["run-lighthouse"] ?? {}) as Record<
      string,
      LighthouseArtifact
    >;
    const siteProgressMap = (siteData?.artifacts["progress"] ?? {}) as Record<
      string,
      CategoryProgress
    >;
    ...
    const allStatuses = Object.values(siteProgressMap).flatMap((cp) =>
      Object.values(cp),
    );
    ...
    const allDone =
      ...
      allStatuses.every((st) => st === "completed" || st === "not-requested");
    ...
    const siteExtractMap = (siteData?.artifacts["extract-pages-content"] ??
      {}) as Record<string, ExtractArtifact>;
    const recordCount = Object.values(siteExtractMap).reduce(
    ...
    return {
      ...
      meta: Object.fromEntries(
        Object.entries(meta).filter(([k]) => k !== "name"),
      ),
    };
  });

  // Order: home → navigation → other categories → contact
  const homeCategory = result.request.categories.find(
    (c) => c.name.toLowerCase() === "home",
  );
  const contactCategory = result.request.categories.find(
    (c) => c.name.toLowerCase() === "contact",
  );
  const otherCategories = result.request.categories.filter(
    (c) =>
      c.name.toLowerCase() !== "home" && c.name.toLowerCase() !== "contact",
  );
  ...
  const sections = [
    ...
    ...(homeCategory
      ? [{ id: `category-${slugify(homeCategory.name)}`, label: homeCategory.name }]
      : []),
    ...otherCategories.map((c) => ({ id: `category-${slugify(c.name)}`, label: c.name })),
    ...(contactCategory
      ? [{ id: `category-${slugify(contactCategory.name)}`, label: contactCategory.name }]
      : []),
  ];
  ...
  function renderCategory(
    ...
    const assessMap = (siteData.artifacts["assess-pages"] ?? {}) as Record<
      ...
      { pages?: unknown[] }
    >;
    const contentPages = (assessMap[cat.id]?.pages ?? []) as Array<{
      url: string;
      pageName: string;
        ...
        <div className="mx-auto max-w-4xl">
          <div className="mb-6">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-foreground-muted">
              ...
              {report?.scrapedAt && (
                <span>
                  Scraped {new Date(report.scrapedAt).toLocaleDateString()}
                </span>
              )}
            </div>
          </div>
          ...
          <NavigationCard
            ...
            categories={result.request.categories.map((c) => ({
              id: c.id,
              name: c.name,
            }))}
          />
